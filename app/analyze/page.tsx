...
function AnalyzeContent() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      setWebsite(ws)
      const hoursDiff = (new Date().getTime() - new Date(ws.updated_at).getTime()) / (1000 * 60 * 60)
      if (hoursDiff < 24 && ws.status === 'indexed') { setNotReady(true); setLoading(false); return }
      const { data: analyticsData } = await supabase.from('analytics').select('*').eq('website_id', websiteId).order('date', { ascending: false })
      const { data: keywordData } = await supabase.from('keywords').select('*').eq('website_id', websiteId).order('date', { ascending: false })
  ...
  const engineData = () => {
    const engines = [
      { name: 'Google', key: 'google' }, { name: 'Bing', key: 'bing' },
      { name: 'DuckDuckGo', key: 'duckduckgo' }, { name: 'Yandex', key: 'yandex' },
      { name: 'Naver', key: 'naver' }, { name: 'Seznam', key: 'seznam' },
      { name: 'Yep', key: 'yep' }, { name: 'Yahoo', key: 'yahoo' },
      { name: 'Ecosia', key: 'ecosia' }, { name: 'Qwant', key: 'qwant' },
    ]
    return engines.map(e => ({
      name: e.name,
      clicks: analytics.filter(a => a.engine === e.key).reduce((s, a) => s + a.clicks, 0),
      impressions: analytics.filter(a => a.engine === e.key).reduce((s, a) => s + a.impressions, 0),
    })).sort((a, b) => b.clicks - a.clicks)
  }

  const countryData = () => {
    const map: Record<string, number> = {}
    analytics.forEach(a => { if (a.country) map[a.country] = (map[a.country] || 0) + a.clicks })
    return Object.entries(map).map(([country, clicks]) => ({ country, clicks })).sort((a, b) => b.clicks - a.clicks).slice(0, 20)
  }

  const stateData = () => {
    const map: Record<string, { clicks: number }> = {}
    analytics.forEach(a => { if (a.region) { const k = `${a.country} / ${a.region}`; if (!map[k]) map[k] = { clicks: 0 }; map[k].clicks += a.clicks } })
    return Object.entries(map).map(([label, val]) => ({ label, clicks: val.clicks })).sort((a, b) => b.clicks - a.clicks).slice(0, 20)
  }

  const trafficData = () => {
    const map: Record<string, number> = {}
    analytics.forEach(a => { map[a.date] = (map[a.date] || 0) + a.clicks })
    return Object.entries(map).map(([date, clicks]) => ({ date, clicks })).sort((a, b) => a.date.localeCompare(b.date)).slice(-30)
  }
      ...
      <div className="main-wrap">
        ...
        {notReady && (
          <div className="card" style={{ textAlign: 'center', padding: 40 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 14 }}>Data will appear 24 hours after indexing. Please check back tomorrow.</p>
          </div>
        )}
        ...
        {!notReady && analytics.length === 0 && (
          <div className="card" style={{ textAlign: 'center', padding: 40 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 14 }}>It can take 24–48 hours for search engine data to appear.</p>
          </div>
        )}
          ...
          <>
            <div className="graph-tabs">
              {([
                ...
                <button key={tab.key} className={`graph-tab ${activeTab === tab.key ? 'active' : ''}`} onClick={() => setActiveTab(tab.key)}>
                  {tab.label}
                </button>
              ))}
            </div>
              ...
              <div className="card">
                ...
                {engineData().map(e => (
                  <div key={e.name} className="bar-item">
                    ...
                    <div className="bar-track"><div className="bar-fill" style={{ width: `${(e.clicks / maxVal(engineData().map(x => x.clicks))) * 100}%` }} /></div>
                    <span className="bar-value">{e.clicks.toLocaleString()}</span>
                  </div>
                ))}
                <p style={{ marginTop: 14, fontSize: 12, color: 'var(--muted)' }}>DuckDuckGo, Yahoo, Ecosia — automatically covered via Bing/IndexNow</p>
              </div>
            )}
              ...
              <div className="card">
                ...
                {countryData().map(c => (
                  <div key={c.country} className="bar-item">
                    ...
                    <div className="bar-track"><div className="bar-fill" style={{ width: `${(c.clicks / maxVal(countryData().map(x => x.clicks))) * 100}%`, background: '#22c55e' }} /></div>
              ...
              <div className="card">
                ...
                {stateData().map(s => (
                  <div key={s.label} className="bar-item">
                    ...
                    <div className="bar-track"><div className="bar-fill" style={{ width: `${(s.clicks / maxVal(stateData().map(x => x.clicks))) * 100}%`, background: '#a855f7' }} /></div>
              ...
              <div className="card">
                ...
                {trafficData().map(t => (
                  <div key={t.date} className="bar-item">
                    ...
                    <div className="bar-track"><div className="bar-fill" style={{ width: `${(t.clicks / maxVal(trafficData().map(x => x.clicks))) * 100}%`, background: '#f59e0b' }} /></div>
...
function CommentsSection({ websiteId, userId }: { websiteId: string; userId: string }) {
  ...
  useEffect(() => {
    const supabase = createClient()
    supabase.from('comments').select('*').eq('website_id', websiteId).order('created_at', { ascending: false })
  ...
  const postComment = async () => {
    ...
    setLoading(true)
    const supabase = createClient()
    await supabase.from('comments').insert({ user_id: userId, website_id: websiteId, content: text, rating })
    setText('')
    setPosted(p => !p)
    setLoading(false)
  }
  ...
  return (
    <div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <textarea value={text} onChange={e => setText(e.target.value)} placeholder="Share your experience with Indexly..." rows={3} style={{ marginBottom: 10, resize: 'none' }} />
